...
_2.default.extend('browserify', function (src, output, baseDir, options) {
    var paths = prepGulpPaths(src, baseDir, output);

    loadPlugins();

    new _2.default.Task('browserify', function (gulp, $) {
        ...
        bundle = function (stream, paths) {
            ...
            return stream.bundle().on('error', function (e) {
                new _2.default.Notification().error(e, 'Browserify Failed!');

                this.emit('end');
            }).pipe(source(paths.output.name)).pipe(buffer()).pipe($.if(config.sourcemaps, $.sourcemaps.init({ loadMaps: true }))).pipe($.if(config.production, $.uglify(config.js.uglify.options))).pipe($.if(config.sourcemaps, $.sourcemaps.write('.'))).pipe(gulp.dest(paths.output.baseDir)).pipe(new _2.default.Notification('Browserify Compiled!'));
        }.bind(this);
...
function prepGulpPaths(src, baseDir, output) {
    return new _2.default.GulpPaths().src(src, baseDir || config.get('assets.js.folder')).output(output || config.get('public.js.outputFolder'), 'bundle.js');
};
